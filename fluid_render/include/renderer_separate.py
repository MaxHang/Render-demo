...
from fluid_render.include.ssf_renderer_separate import SSFRenderer
...
class Renderer:
    ...
    def __init__(self, camera:Camera, light=None) -> None:
        ...
        self.m_particle_vao1    = None
        self.m_particle_vao2    = None
        self.m_particle_vao3    = None
        self.m_particle_vao4    = None
    ...
    def render(self, path:str, k_fluid:list):
        ...
        # vertices, vertices_stride = read_ply(path, self.m_ssf_renderer.m_shader_option, k_fluid)
    ...
    def render_k_fluid(self, path:str, k_fluid:int=1):
        """渲染一帧
    ...
    # def __render_back_ground(self)


    def __render(self, path:str, k_fluid:list):
        """渲染一帧, OpenGL相关"""
        # 得到变换矩阵
        ...
        if not self.m_ssf_fluid_draw_flag:
            ...
            for idx, k_index in enumerate(k_fluid):
                first_k_index = idx == 0
                # 读取并绑定粒子数据
                read_start = time.perf_counter()
                vertices, vertices_stride = read_ply(path, self.m_ssf_renderer.m_shader_option, k_index)
                self.read_time = time.perf_counter() - read_start
                # particle_data_start = time.perf_counter()
                self.m_particle_num = len(vertices) // vertices_stride
                self.m_particle_vbo.set_vbo_data(data=vertices)
                ...
                if vertices_stride > 3:
                    ...
                    gl.glVertexAttribPointer(1, vertices_stride - 3, gl.GL_FLOAT, gl.GL_FALSE, vertices_stride * sizeof(c_float), c_void_p(3 * sizeof(c_float)))
                gl.glBindVertexArray(0)
                self.m_ssf_renderer.render(self.m_particle_vao, self.m_particle_num, point_scale, self.m_back_ground_texture, first_k_index)
...
def read_ply(path:str, shader_option:ShaderOption, k_fluid:int=1):
    ...
    if f'fluid{k_fluid}_frac' in properties:
        k_particle_index_start = (k_fluid - 1) * 2000
        k_particle_index_end = (k_fluid) * 2000
    ...
    if shader_option in [ShaderOption.MultiFluid, ShaderOption.MultiFrac, ShaderOption.MultiAttenuation, ShaderOption.MultiRefract]:
        ...
        vertices_stride = 3
        for i in range(1, 5):
            if f'fluid{i}_frac' in properties and i <= k_fluid:
                vertices_stride += 1
        ...
        vertices = np.asarray([
            vertices_render['vertex']['x'], 
            vertices_render['vertex']['y'], 
            vertices_render['vertex']['z'],
