...
from fluid_render.include.ssf_renderer_separate_v2 import SSFRenderer
from fluid_render.include.static_variable import far, near, particle_radius, skybox_faces, skybox_vertices, lamp_vertices, ground_vertices, use_set_activate_flag, set_activate_values
...
class Renderer:
    ...
    def __init(self):
        ...
        '''ssf流体的渲染颜色附件'''
        self.m_ssf_texture1 = create_texture_2D(
            internal_format=gl.GL_RGB32F,
            width=self.m_width,
            height=self.m_height,
            src_format=gl.GL_RGB,
        ...
        self.m_ssf_texture2 = create_texture_2D(
            internal_format=gl.GL_RGB32F,
            width=self.m_width,
            height=self.m_height,
            src_format=gl.GL_RGB,
        ...
        self.m_ssf_texture3 = create_texture_2D(
            internal_format=gl.GL_RGB32F,
            width=self.m_width,
            height=self.m_height,
            src_format=gl.GL_RGB,
        ...
        self.m_ssf_texture4 = create_texture_2D(
            internal_format=gl.GL_RGB32F,
            width=self.m_width,
            height=self.m_height,
            src_format=gl.GL_RGB,
        ...
        '''包括背景的ssf流体的渲染颜色附件'''
        # self.m_ssf_fluid1 = create_texture_2D(
        #     internal_format=gl.GL_RGB32F,
        #     width=self.m_width,
        #     height=self.m_height,
        #     src_format=gl.GL_RGB,
        #     src_data_type=gl.GL_FLOAT
        # )
        # self.m_ssf_fluid2 = create_texture_2D(
        #     internal_format=gl.GL_RGB32F,
        #     width=self.m_width,
        #     height=self.m_height,
        #     src_format=gl.GL_RGB,
        #     src_data_type=gl.GL_FLOAT
        # )
        # self.m_ssf_fluid3 = create_texture_2D(
        #     internal_format=gl.GL_RGB32F,
        #     width=self.m_width,
        #     height=self.m_height,
        #     src_format=gl.GL_RGB,
        #     src_data_type=gl.GL_FLOAT
        # )
        # self.m_ssf_fluid4 = create_texture_2D(
        #     internal_format=gl.GL_RGB32F,
        #     width=self.m_width,
        #     height=self.m_height,
        #     src_format=gl.GL_RGB,
        ...
        gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT1, gl.GL_TEXTURE_2D, self.m_ssf_texture1, 0)  
        gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT2, gl.GL_TEXTURE_2D, self.m_ssf_texture2, 0)  
        gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT3, gl.GL_TEXTURE_2D, self.m_ssf_texture3, 0)  
        gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT4, gl.GL_TEXTURE_2D, self.m_ssf_texture4, 0)  

        # gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT5, gl.GL_TEXTURE_2D, self.m_ssf_fluid1, 0)  
        # gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT6, gl.GL_TEXTURE_2D, self.m_ssf_fluid2, 0)  
        # gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT7, gl.GL_TEXTURE_2D, self.m_ssf_fluid3, 0)  
        # gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT8, gl.GL_TEXTURE_2D, self.m_ssf_fluid4, 0)  
        ...
        self.m_quad_render_shader = Shader('quad_render.vs', 'quad_render.fs')
        # 1.particle
        ...
        self.m_quad_vao = self.m_ssf_renderer.m_quad_vao
    ...
    def render(self, path:str, k_fluid:list):
        """渲染一帧
        ...
        if not glfw.window_should_close(self.m_window):
            ...
            '''开始SSF的渲染, 对于k相流体渲染k遍, 但是背景确实一样的, 故前面渲染背景的代码只执行一次'''
            avg_fluid_values = 1.0 / len(k_fluid)
            if len(k_fluid) == 4:
                activate_ssf_fluid_values = glm.vec4(avg_fluid_values, avg_fluid_values, avg_fluid_values, avg_fluid_values)
            elif len(k_fluid) == 3:
                activate_ssf_fluid_values = glm.vec4(avg_fluid_values, avg_fluid_values, avg_fluid_values, 0)
            elif len(k_fluid) == 2:
                activate_ssf_fluid_values = glm.vec4(avg_fluid_values, avg_fluid_values, 0, 0)
            if use_set_activate_flag:
                activate_ssf_fluid_values = set_activate_values
            for k_fluid_index in k_fluid:
                '''读取流相粒子, 并设置vao'''
                vertices, vertices_stride = read_ply(path, self.m_ssf_renderer.m_shader_option, k_fluid_index)
                ...
                gl.glBindVertexArray(0)
                '''计算SSF应该渲染到第几个颜色附件'''
                Index_color_attchment_map = {
                    1: gl.GL_COLOR_ATTACHMENT1,
                    2: gl.GL_COLOR_ATTACHMENT2,
                    3: gl.GL_COLOR_ATTACHMENT3,
                    4: gl.GL_COLOR_ATTACHMENT4,
                }
                ssf_color_attachment_rendered = Index_color_attchment_map.get(k_fluid_index)

                '''执行ssf渲染, 包括流体渲染以及背景渲染, 渲染到上述计算的颜色附件中'''
                self.__render(k_fluid_index, ssf_color_attachment_rendered, light_position)
            ...
            self.m_quad_render_shader.use()

            self.m_quad_render_shader.set_int('fluid1SSFTexture', 0)
            self.m_quad_render_shader.set_int('fluid2SSFTexture', 1)
            self.m_quad_render_shader.set_int('fluid3SSFTexture', 2)
            self.m_quad_render_shader.set_int('fluid4SSFTexture', 3)      
            self.m_quad_render_shader.set_vec4('activateSSFFluid', activate_ssf_fluid_values)      

            gl.glActiveTexture(gl.GL_TEXTURE0)
            gl.glBindTexture(gl.GL_TEXTURE_2D, self.m_ssf_texture1)
            gl.glActiveTexture(gl.GL_TEXTURE1)
            gl.glBindTexture(gl.GL_TEXTURE_2D, self.m_ssf_texture2)
            gl.glActiveTexture(gl.GL_TEXTURE2)
            gl.glBindTexture(gl.GL_TEXTURE_2D, self.m_ssf_texture3)
            gl.glActiveTexture(gl.GL_TEXTURE3)
            gl.glBindTexture(gl.GL_TEXTURE_2D, self.m_ssf_texture4)
    ...
    def __render(self, k_fluid_index:int, ssf_color_attachment_rendered:int, light_position:glm.vec3):
        """渲染一帧, OpenGL相关"""
        # 得到变换矩阵
        zoom = self.m_camera.get_zoom()
        ...
        if not self.m_ssf_fluid_draw_flag:
            ...
            self.m_ssf_renderer.render(
                self.m_particle_vao, 
                self.m_particle_num, 
                point_scale, 
                self.m_back_ground_texture,
                self.m_back_ground_fbo,
                k_fluid_index,
                ssf_color_attachment_rendered
            )
...
def read_ply(path:str, shader_option:ShaderOption, k_fluid_index:int=1):
    ...
    single_fluid_particle_num = 2541
    if f'fluid{k_fluid_index}_frac' in properties:
        k_particle_index_start = (k_fluid_index - 1) * 2541
        k_particle_index_end = k_fluid_index * 2541
        vertices_render = plydata['vertex'][k_particle_index_start: k_particle_index_end]
        pass
    else:
        print('没有该流相')
        exit()
    ...
    if shader_option in [ShaderOption.MultiFluid, ShaderOption.MultiFrac, ShaderOption.MultiAttenuation, ShaderOption.MultiRefract]:
        ...
        vertices_stride = 3
        for i in range(1, 5):
            if f'fluid{i}_frac' in properties and i <= k_fluid_index:
                vertices_stride += 1
                vertices_data.append(vertices_render[f'fluid{i}_frac'])
            else:
                break
...
#     if f'fluid{k_fluid_index}_frac' in properties:
#         if k_fluid_index == 1:
#             k_particle_index_start = 0
#             k_particle_index_end = 12493
#         else:
#             k_particle_index_start = 12493
#             k_particle_index_end = 12813
#         vertices_render = plydata['vertex'][k_particle_index_start: k_particle_index_end]
#         pass
#     else:
#         print('没有该流相')
