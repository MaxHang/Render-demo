...
class SSFRenderer:
    ...
    def __init__(self, width:int, height:int, camera:Camera, sky_texture:int) -> None:
        ...
        :m_fbo_rendered: 渲染保存到的帧缓冲
        :m_color_attachment_rendered: 渲染保存到的颜色附件
        '''
        self.m_fbo_rendered = None
        self.m_color_attachment_rendered = None
    ...
    def __init(self):
        ...
        self.m_rendering_shader          = Shader('ssf_rendering.vs', 'ssf_rendering_separate.fs')
    ...
    def render(self, particle_vao:int, partical_num:int, point_scale:float, back_ground_texture:int, fbo_rendered:int, k_fluid_index:int, color_attachment_rendered:int):
        """SSF将粒子渲染为光滑流体"""
        self.m_particle_vao = particle_vao
        self.m_particle_num = partical_num
        self.m_point_scale = point_scale
        self.m_fbo_rendered = fbo_rendered
        self.m_color_attachment_rendered = color_attachment_rendered
        ...
        self.__render(back_ground_texture, k_fluid_index)
        gl.glEndQuery(gl.GL_TIME_ELAPSED)
    ...
    def __restore_normal(self):
        ...
        gl.glBindFramebuffer(gl.GL_DRAW_FRAMEBUFFER, self.m_fbo_rendered) # 写人默认帧缓冲中
        gl.glBlitFramebuffer(0,0,self.m_width,self.m_height,0,0,self.m_width,self.m_height,gl.GL_DEPTH_BUFFER_BIT,gl.GL_NEAREST)

    def __render(self, back_ground_texture:int, k_fluid_index:int):
        ...
        self.m_rendering_shader.set_int('kFluidIndex', k_fluid_index)
        ...
        gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, self.m_fbo_rendered)
        render_buffers = [self.m_color_attachment_rendered]
        render_buffers = (c_uint32 * len(render_buffers))(*render_buffers)
        gl.glDrawBuffers(1, render_buffers)
        gl.glDisable(gl.GL_DEPTH_TEST)
        gl.glDisable(gl.GL_BLEND)
