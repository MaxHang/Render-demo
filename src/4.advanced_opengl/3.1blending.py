...
def main():
    ...
    shader = Shader("src/4.advanced_opengl/shaders/3.1.blending.vs", "src/4.advanced_opengl/shaders/3.1.blending.fs")
    ...
    while not glfw.window_should_close(window):
        ...
        for veg_pos in vegetation:
            model = glm.mat4(1.0)
            model = glm.translate(model, veg_pos)
            shader.set_mat4("model", glm.value_ptr(model))
            gl.glDrawArrays(gl.GL_TRIANGLES, 0, 6)
