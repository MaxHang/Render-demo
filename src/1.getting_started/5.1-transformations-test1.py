...
def main():
    ...
    shader = Shader(r'res/shaders/5-transformations.shader')
    ...
    while not glfw.window_should_close(window):
        ...
        transform = glm.rotate(transform, glfw.get_time(), [0.0, 0.0, 1.0])

        shader.set_mat4('transform', glm.value_ptr(transform))
        ...
        transform = glm.mat4(1.0)
        transform = glm.translate(transform, [-0.5, 0.5, 0.0])
        scale_amount = math.sin(glfw.get_time())
        transform = glm.scale(transform, [scale_amount] * 3)
